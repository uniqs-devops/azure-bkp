...
COPY src/avamar/*.sh /dockerclient
RUN chmod 755 /dockerclient/*.sh
#RUN /dockerclient/setup.sh
# Cleanup /tmp folder, agent start  and Configuration persist
RUN rm -f /tmp/*.rpm
ENTRYPOINT mount -a &&  [ -f /etc/init.d/avagent ] && /etc/init.d/avagent start && /dockerclient/hosts.sh && /bin/bash
