...
RUN rm -f /tmp/*.rpm
#CMD echo localhost localhost.localdomain dockerpg-01.pcalvo.local > /etc/hosts; supervisord -n;
ENTRYPOINT mount -a &&  [ -f /etc/init.d/avagent ] && /etc/init.d/avagent start && /dockerclient/setup.sh && /bin/bash
